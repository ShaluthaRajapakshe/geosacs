...
class IKinterface(): 
    def __init__(self):
        rospy.init_node("ik_interface_pos") 
        ...
        rospy.Subscriber("/task_end", String, self.task_end_cb)
        ...
        #Init Message
        rospy.loginfo("ik_interface_pose has been started") 



    def task_end_cb(self, msg):
        if msg.data == "True":
            self.task_end = True
            # print("Received True message")
    ...
    def ik_cb(self, msg):
        ...
        # print("IK vals", self.ik_js)
    # else:
        #     print("task end by ik_cb")
            # print("received ik_js: ", self.ik_js)

    def ik_cb_end(self, msg):

        if self.task_end:  #try without the if condition as well
