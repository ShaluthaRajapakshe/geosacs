...
#include "custom_msgs/WeightedPose.h"
...
int main(int argc, char **argv) {
    ...
    bool start = true;
    bool run = true;
    // Adjust the following variables depending on if you want dynamic weight change or not
    double q_weight = 1.;
    bool weighted_pose = false;

    ros::Subscriber twistSub1 = nh.subscribe<custom_msgs::WeightedPose>("/weighted_pose", 10,
    [&](const custom_msgs::WeightedPose::ConstPtr& msg) {
        if (weighted_pose){
