...
class PoseControllerNode():
    def __init__(self):
        ...
        if self.task == "marshmellow":
            ...
            # self.secondary_joint_positions = [x,x,x,x,x,x]   ## Add after checking the joint states
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.first:
                ...
                # print("Before joint positions", self.joint_positions)
                ...
                if self.task == "marshmellow" and self.marsh_selected: 
                    ...
                    print ("New values for picking the marshmallow", pos_vals)

                    print("current joint positions", self.joint_positions)

                    msg1 = PoseStamped()
                    msg1.header.frame_id = "LIO_base_link"
                    # msg1.pose.position.x = self.lio_pose.pose.position.x
                    msg1.pose.position.x = pos_vals[0]
                    msg1.pose.position.y = pos_vals[1]
                    msg1.pose.position.z = pos_vals[2]

                    msg1.pose.orientation.w = ori_vals[0]
                    msg1.pose.orientation.x = ori_vals[1]
                    msg1.pose.orientation.y = ori_vals[2]
                    msg1.pose.orientation.z = ori_vals[3]


                    count = 0
                    ''' As the IK engine calculates values w.r.t the LIO_base_link (that is the parent link in the URDF file provided to the IK engine), 
                        we have to publish the values in the same frame as well 
                    '''
                    self.commanded_pose_pub.publish(msg1)  # publishing to the ik engine
            ...
            if self.terminate:
                ...
                rospy.loginfo(f"Total correction distance: {self.cumulative_correction_distance} meters")
                rospy.loginfo(f"Total Lio correction distance: {self.lio_cumulative_correction_distance} meters")
                rospy.loginfo(f"Correction time as a percentage from total: {self.cumulative_correction_time} seconds ({(self.cumulative_correction_time/task_duration)*100}%)")
