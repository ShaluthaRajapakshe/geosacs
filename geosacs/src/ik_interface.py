...
class IKinterface(): 
    def __init__(self):
        ...
        rospy.Subscriber("/lio_1c/sensors", Sensors, self.lio_sensor_cb)

        #Init Message
        rospy.loginfo("ik_interface has been started") 

    def lio_sensor_cb(self, msg):
        return
        distance = msg.data[8]
        readable = msg.readable[8]

        if not readable and not self.gripper_closed: return
        elif not readable and self.gripper_closed:
            self.gripper_closed = False
        elif readable and not self.gripper_closed:
            if distance<10.5: 
                self.gripper_angle = 0
                self.gripper_closed = True
